...
/obj/item/food/canned/proc/open_can(mob/user)
	...
	playsound(user.loc, 'oworld/sound/food/foodcan_preopen.wav', 65)
	if(!do_after(user, 5, src))
		return
	to_chat(user, span_notice("I pull back the lid of \the [src]."))
	playsound(user.loc, 'oworld/sound/food/foodcan_open.wav', 65)
...
/obj/item/food/canned/beef
	...
	icon = 'oworld/icons/obj/food/canned.dmi'
	icon_state = "beef"
