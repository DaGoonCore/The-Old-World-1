...
/datum/job/proc/assign_attributes(mob/living/spawned, client/player_client)
	...
	if(SSmapping.config?.combat_map)
		...
		if(!babble)
			spawned_human.AddComponent(/datum/component/babble, 'oworld/sound/voice/babble/gakster.wav')
		else
			babble.babble_sound_override = 'oworld/sound/voice/babble/gakster.wav'
