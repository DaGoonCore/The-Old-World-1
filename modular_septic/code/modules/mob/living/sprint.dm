...
/mob/living/proc/toggle_sprint()
	...
	if(usable_legs < default_num_legs)
		...
		return
	if(CHECK_BITFIELD(combat_flags, COMBAT_FLAG_SPRINT_ACTIVE))
		playsound_local(src, 'oworld/sound/effects/sprintactivate.ogg', 60, FALSE, pressure_affected = FALSE)
		enable_sprint()
	else
		playsound_local(src, 'oworld/sound/effects/sprintdeactivate.ogg', 60, FALSE, pressure_affected = FALSE)
		disable_sprint()
	hud_used?.sprint?.update_appearance()
	hud_used?.mov_intent?.update_appearance()
