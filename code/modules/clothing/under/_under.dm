...
/obj/item/clothing/under/worn_overlays(mutable_appearance/standing, isinhands = FALSE)
	...
	*/
	...
	//OWORLD EDIT BEGIN
	if(damaged_clothes)
		var/mutable_appearance/damage_appearance = mutable_appearance('oworld/icons/effects/item_damage.dmi', "damageduniform")
		damage_appearance.color = "#000000"
		. += damage_appearance
	if(HAS_BLOOD_DNA(src))
		. += mutable_appearance('oworld/icons/effects/blood.dmi', "uniformblood")
	if(HAS_SHIT_DNA(src))
		var/mutable_appearance/shit_appearance = mutable_appearance('oworld/icons/effects/shit.dmi', "uniformshit")
		shit_appearance.color = COLOR_BROWN_SHIT
		. += shit_appearance
	if(HAS_CUM_DNA(src))
		var/mutable_appearance/cummy = mutable_appearance('oworld/icons/effects/cum.dmi', "uniformcum")
		cummy.color = COLOR_WHITE_CUM
		. += cummy
	if(HAS_FEMCUM_DNA(src))
		var/mutable_appearance/femcummy = mutable_appearance('oworld/icons/effects/femcum.dmi', "uniformfemcum")
