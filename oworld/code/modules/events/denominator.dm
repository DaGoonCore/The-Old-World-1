...
/proc/spawn_denominators(datum/comm_message/threat, ship_template, skip_answer_check)
	...
	playsound_local(candidates, 'oworld/sound/effects/ghostcue.wav', 60)
...
/obj/item/keycard/red
	...
	icon_state = "redkey"
	puzzle_id = "denom"
	pickup_sound = 'oworld/sound/effects/card_pickup.wav'
	drop_sound = 'oworld/sound/effects/card_drop.wav'
...
/obj/machinery/door/keycard/denominator
	...
	icon = 'oworld/icons/obj/machinery/tall/doors/airlocks/secretdoor.dmi'
...
/obj/machinery/door/keycard/denominator/inborn
	name = "yellow airlock"
	desc = "This door only opens when a keycard is swiped. It looks like It's been heavily armored."
	icon = 'oworld/icons/obj/machinery/tall/doors/airlocks/secretdoor_yellow.dmi'
	icon_state = "door_closed"
	base_icon_state = "door"
	/// Make sure that the key has the same puzzle_id as the keycard door!
	puzzle_id = "InBor"
...
/obj/machinery/door/keycard/denominator/attackby(obj/item/I, mob/user, params)
	if(istype(I,/obj/item/keycard))
		...
		if((!puzzle_id || puzzle_id == key.puzzle_id)  && density)
			...
			playsound(src, 'oworld/sound/effects/card_accepted.wav', 70, FALSE, 3)
			sleep(6)
			playsound(src, 'oworld/sound/effects/secretopen1.wav', 70, FALSE, 4)
			open()
			return
			...
			playsound(src, 'oworld/sound/effects/card_accepted.wav', 70, FALSE, 3)
			sleep(6)
			playsound(src, 'oworld/sound/effects/secretclose2.wav', 70, FALSE, 4)
			close()
			return
		else
			to_chat(user, span_notice("[src] buzzes. This must not be the right key."))
			playsound(src, 'oworld/sound/effects/card_declined.wav', 60, FALSE, 1)
			return
