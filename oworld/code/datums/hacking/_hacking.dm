...
/datum/hacking/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("start_hacking")
			...
			playsound(holder, 'oworld/sound/hacking/ddos_start.wav', 80, FALSE)
		if("do_attack")
...
/datum/hacking/proc/start_hacking(hacking_action = "Destroy", mob/living/hackerman)
	if(hacked)
		playsound(holder, 'oworld/sound/hacking/ddos_success.wav', 80, FALSE)
...
/datum/hacking/proc/hacker_attack(attack_type = "Attack")
	...
	playsound(holder, "oworld/sound/hacking/ddos[rand(1, 4)].wav", 80, FALSE)
	switch(attack_type)
		if("Attack")
...
/datum/hacking/proc/hacker_loss()
	playsound(holder, "oworld/sound/hacking/ddos_failure.wav", 80, FALSE)
	if(hackingtool)
		if(ishackingtool(hackingtool))
			hackingtool.cut_overlays()
